...
def show_medicine_view(parent):
    ...
    right = ctk.CTkFrame(container, fg_color="white", corner_radius=10, width=420)
    ...
    search_entry = ctk.CTkEntry(search_frame, placeholder_text="Search medicines...", width=300)
    ...
    def load_meds(query=""):
        ...
        if not meds:
            ctk.CTkLabel(med_container, text="No medicines registered.", text_color="gray").pack(pady=20)
            return
        ...
        for m in meds:
            ...
            ctk.CTkLabel(card, text=f"{m['name']}", font=("Arial", 13, "bold"), anchor="w").grid(row=0, column=0, sticky="w", padx=10, pady=(8,2))
            ctk.CTkLabel(card, text=f"Stock: {m['stock']} | ₱{float(m['price']):,.2f}", font=("Arial", 11), anchor="w").grid(row=1, column=0, sticky="w", padx=10, pady=(0,8))
    ...
    ctk.CTkButton(search_frame, text="Search", width=80, command=lambda: load_meds(search_entry.get())).pack(side="left", padx=5)
    ctk.CTkButton(search_frame, text="Clear", width=80, command=lambda: [search_entry.delete(0, 'end'), load_meds()]).pack(side="left")
    ctk.CTkButton(search_frame, text="Refresh", width=80, command=refresh).pack(side="left", padx=5)

    # Right form
    ctk.CTkLabel(right, text="Medicine Details", font=("Arial", 20, "bold")).pack(pady=15)
    ...
    ctk.CTkLabel(form, text="Price (₱):").pack(anchor='w')
    price_entry = ctk.CTkEntry(form)
    price_entry.pack(fill='x', pady=6)

    ctk.CTkLabel(form, text="Form:").pack(anchor='w')
    form_combo = ctk.CTkComboBox(form, values=["Tablet", "Injection", "Syrup", "Cream", "Ointment", "Drops", "Suppository", "Other"], width=200)
    form_combo.set("Tablet")
    form_combo.pack(fill='x', pady=6)

    ctk.CTkLabel(form, text="Supplier Name:").pack(anchor='w')
    supplier_entry = ctk.CTkEntry(form)
    supplier_entry.pack(fill='x', pady=6)

    ctk.CTkLabel(form, text="Supplier Contact:").pack(anchor='w')
    supplier_contact_entry = ctk.CTkEntry(form)
    ...
    ctk.CTkButton(btn_frame, text='Save', command=save_med, fg_color='#2ecc71').pack(side='left', expand=True, fill='x', padx=4)
    ctk.CTkButton(btn_frame, text='New', command=clear_form, fg_color='#3498db').pack(side='left', expand=True, fill='x', padx=4)
    ctk.CTkButton(btn_frame, text='Delete', command=delete_med, fg_color='#e74c3c').pack(side='left', expand=True, fill='x', padx=4)
    ctk.CTkButton(btn_frame, text='Load Sample Medicines', command=load_sample_meds, fg_color='#9b59b6').pack(side='left', expand=True, fill='x', padx=4)
