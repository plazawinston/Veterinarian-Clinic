...
def show_medicine_view(parent):
    ...
    right = ctk.CTkFrame(container, fg_color="white", corner_radius=10, width=420 + module_scale * 8)
    ...
    search_entry = ctk.CTkEntry(search_frame, placeholder_text="Search medicines...", width=300 + module_scale * 8, font=F(12))
    ...
    def load_meds(query=""):
        ...
        if not meds:
            ctk.CTkLabel(med_container, text="No medicines registered.", text_color="gray", font=F(12)).pack(pady=20)
            return
        ...
        for m in meds:
            ...
            ctk.CTkLabel(card, text=f"{m['name']}", font=F(13, "bold"), anchor="w").grid(row=0, column=0, sticky="w", padx=10, pady=(8,2))
            ctk.CTkLabel(card, text=f"Stock: {m['stock']} | ₱{float(m['price']):,.2f}", font=F(11), anchor="w").grid(row=1, column=0, sticky="w", padx=10, pady=(0,8))
            ...
            def on_click(e=None, mid=m['id'], card_ref=card):
                ...
                uses_entry.delete(0, 'end'); uses_entry.insert(0, med.get('use', "") or "")
                form_combo.set(med.get('form', "Tablet"))
                supplier_entry.delete(0, 'end'); supplier_entry.insert(0, med.get('supplier_name', "") or "")
                supplier_contact_entry.delete(0, 'end'); supplier_contact_entry.insert(0, med.get('supplier_contact', "") or "")
                # debug trace
                print(f"Loaded medicine: {med['name']}, Use: {med.get('use','')}, Supplier: {med.get('supplier_name','')}, Contact: {med.get('supplier_contact','')}")
    ...
    ctk.CTkButton(search_frame, text="Search", width=80 + module_scale * 4, height=34 + module_scale, command=lambda: load_meds(search_entry.get())).pack(side="left", padx=5)
    ctk.CTkButton(search_frame, text="Clear", width=80 + module_scale * 4, height=34 + module_scale, command=lambda: [search_entry.delete(0, 'end'), load_meds()]).pack(side="left", padx=5)
    ctk.CTkButton(search_frame, text="Refresh", width=80 + module_scale * 4, height=34 + module_scale, command=refresh).pack(side="left", padx=5)

    # Right form
    ctk.CTkLabel(right, text="Medicine Details", font=F(20, "bold")).pack(pady=15)

    form = ctk.CTkFrame(right, fg_color="transparent")
    form.pack(fill="both", expand=True, padx=10, pady=10)

    ctk.CTkLabel(form, text="Name:", font=F(12)).pack(anchor='w', pady=(4,0))
    name_entry = ctk.CTkEntry(form, font=F(12))
    name_entry.pack(fill='x', pady=6)

    ctk.CTkLabel(form, text="Uses:", font=F(12)).pack(anchor='w', pady=(4,0))
    uses_entry = ctk.CTkEntry(form, font=F(12))
    uses_entry.pack(fill='x', pady=6)

    ctk.CTkLabel(form, text="Stock:", font=F(12)).pack(anchor='w', pady=(4,0))
    stock_entry = ctk.CTkEntry(form, font=F(12))
    stock_entry.pack(fill='x', pady=6)

    ctk.CTkLabel(form, text="Price (₱):", font=F(12)).pack(anchor='w', pady=(4,0))
    price_entry = ctk.CTkEntry(form, font=F(12))
    price_entry.pack(fill='x', pady=6)

    ctk.CTkLabel(form, text="Form:", font=F(12)).pack(anchor='w', pady=(4,0))
    form_combo = ctk.CTkComboBox(form, values=["Tablet", "Injection", "Syrup", "Cream", "Ointment", "Drops", "Suppository", "Other"], width=200 + module_scale*6, font=F(11))
    form_combo.set("Tablet")
    form_combo.pack(fill='x', pady=6)

    ctk.CTkLabel(form, text="Supplier Name:", font=F(12)).pack(anchor='w', pady=(4,0))
    supplier_entry = ctk.CTkEntry(form, font=F(12))
    supplier_entry.pack(fill='x', pady=6)

    ctk.CTkLabel(form, text="Supplier Contact:", font=F(12)).pack(anchor='w', pady=(4,0))
    supplier_contact_entry = ctk.CTkEntry(form, font=F(12))
    ...
    ctk.CTkButton(btn_frame, text='Save', command=save_med, fg_color='#2ecc71', height=40 + module_scale, font=F(13, "bold")).pack(side='left', expand=True, fill='x', padx=4)
    ctk.CTkButton(btn_frame, text='New', command=clear_form, fg_color='#3498db', height=40 + module_scale, font=F(13)).pack(side='left', expand=True, fill='x', padx=4)
    ctk.CTkButton(btn_frame, text='Delete', command=delete_med, fg_color='#e74c3c', height=40 + module_scale, font=F(13)).pack(side='left', expand=True, fill='x', padx=4)
    ctk.CTkButton(btn_frame, text='Load Sample Medicines', command=load_sample_meds, fg_color='#9b59b6', height=40 + module_scale, font=F(12)).pack(side='left', expand=True, fill='x', padx=4)
